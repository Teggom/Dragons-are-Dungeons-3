...
class Tile:
    ...
    """
    def __init__(self, blocked, palette_type, block_sight=None, palette_seed = None):
    ...
    def step(self, player):
        self.last_seen += 1
        if self.last_seen / (4**player.stats.intelligence + self.last_seen) > random():
            self.explored = False
            self.last_seen = 0
            return(True)
        return(False)
    
    def update_tile(self, blocked, palette_type, block_sight=None, palette_seed=None):
        self.blocked = blocked
        self.set_palette(palette_type, palette_seed)
        # By default, if a tile is blocked, it also blocks sight
        if block_sight is None:
